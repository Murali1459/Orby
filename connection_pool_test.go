...
func TestConnectionPoolReportsOnlyCurrentLeases(t *testing.T) {
	...
	if err := pool.Connect("redis.internal:6379", "second", factory); err != nil {
		t.Fatal(err)
	}
	if err := pool.Connect("redis.internal:6379", "first", factory); err != nil {
		t.Fatal(err)
	}
...
func TestConnectionPoolReusesHostPortAcrossLeases(t *testing.T) {
	...
	if err := pool.Connect("redis.internal:6379", "user-a", factory); err != nil {
		t.Fatal(err)
	}
	if err := pool.Connect("redis.internal:6379", "user-b", factory); err != nil {
		t.Fatal(err)
	}
	acquired, release, err := pool.Acquire("redis.internal:6379", "user-b", factory)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConnectionPoolAcquireExistingRequiresConnectedLease(t *testing.T) {
	pool := newConnectionPool(time.Minute)
	if _, _, err := pool.AcquireExisting("redis.internal:6379", "preset:redis-ci"); !errors.Is(err, errConnectionNotConnected) {
		t.Fatalf("missing connection error = %v", err)
	}

	connection := &fakePooledConnection{}
	if err := pool.Connect("redis.internal:6379", "preset:redis-ci", func() (pluginapi.Connection, error) { return connection, nil }); err != nil {
		t.Fatal(err)
	}
	acquired, release, err := pool.AcquireExisting("redis.internal:6379", "preset:redis-ci")
...
func TestConnectionPoolCreatesOnceForConcurrentConnects(t *testing.T) {
	...
	for _, lease := range []string{"one", "two"} {
		...
		go func(lease string) {
			defer wait.Done()
			errorsSeen <- pool.Connect("redis.internal:6379", lease, factory)
		}(lease)
	}
	for creates.Load() == 0 {
...
func TestConnectionPoolDefersCloseUntilActiveRequestFinishes(t *testing.T) {
	...
	connection := &fakePooledConnection{}
	factory := func() (pluginapi.Connection, error) { return connection, nil }
	if err := pool.Connect("redis.internal:6379", "lease", factory); err != nil {
		t.Fatal(err)
	}
	_, release, err := pool.Acquire("redis.internal:6379", "lease", factory)
	if err != nil {
		t.Fatal(err)
	}
...
func TestConnectionPoolEvictsIdleAndClosesOnShutdown(t *testing.T) {
	...
	if err := pool.Connect("one:6379", "one", func() (pluginapi.Connection, error) { return first, nil }); err != nil {
		t.Fatal(err)
	}
	if err := pool.Connect("two:3000", "two", func() (pluginapi.Connection, error) { return second, nil }); err != nil {
		t.Fatal(err)
	}
	...
	if err := pool.Connect("three:6379", "three", func() (pluginapi.Connection, error) { return third, nil }); err != nil {
		t.Fatal(err)
	}
	pool.Close()
...
func TestConnectionPoolAutomaticallyClosesIdleConnection(t *testing.T) {
	...
	defer pool.Close()
	connection := &fakePooledConnection{}
	if err := pool.Connect("redis.internal:6379", "lease", func() (pluginapi.Connection, error) {
		return connection, nil
	}); err != nil {
		t.Fatal(err)
	}
